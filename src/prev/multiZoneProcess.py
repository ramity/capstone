...
def main():
    ...
    maskContours = cv2.drawContours(blank, contours, -1, (0, 255, 0), 1)
    cv2.imwrite("./output/maskContours.jpg", maskContours)
        ...
        if hierarchy[id][3] < 0:
            ...
            for z in range(0, rows):
                ...
                if confidence != -1:
                    ...
                    cv2.putText(final, text, (finalX, finalY), font, fontScale, color, lineType)
                    textWidth = size[0][0]
                    textHeight = size[0][1]
                    cv2.putText(final, str(confidence), (finalX + textWidth, finalY + textHeight), font, fontScale, (0, 255, 0), lineType)
