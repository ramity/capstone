...
finalGray = np.zeros([image.shape[0], image.shape[1]])
...
for contour in contours:
    ...
    if(M['m00']):
        ...
        finalGray[y:y+h,x:x+w] = cv2.cvtColor(image[y:y+h,x:x+w], cv2.COLOR_RGB2GRAY)
        final[y:y+h,x:x+w] = image[y:y+h,x:x+w]
        face = gray[y:y+h,x:x+w]
        rect, face = cv2.threshold(face, 0, 175, cv2.THRESH_BINARY)

        faces = 3
        contours, hierarchy = cv2.findContours(finalGray[y:y+h,x:x+w], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contours = sorted(contours, key=cv2.contourArea, reverse=True)[:faces]
        rect = cv2.minAreaRect(contour)
        box = cv2.boxPoints(rect)
        cv2.drawContours(final[y:y+h,x:x+w],[box],0,(0,0,255),2)
...
cv2.imwrite("./output/finalGray.jpg", finalGray)
